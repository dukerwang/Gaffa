...
- **Title Case for headings, labels, and buttons**: Headings, section titles, button labels, stat keys, and short UI labels use **Title Case** ("Record Book", "Title-Winning XI", "Save Lineup", "Manage Squad", "Active Roster", "Injured Reserve") — decided 2026-09-09, see `docs/DECISIONS.md`. Running prose stays sentence case: full sentences in help text, error messages, and state notices read as sentences ("Over the cap. Drop or move a player out before the next signing."). Labels already uppercased in CSS are unaffected.
...
- **A player who arrives at a full squad is `held`, never auto-dropped or pushed over the limit** (migrations 161–164, `src/lib/roster/holds.ts`, spec `docs/superpowers/specs/2026-09-13-held-players-design.md`). Held is uncounted (`UNCOUNTED_ROSTER_STATUSES` and SQL `team_active_count` must agree), activated only by the manager and never mid-gameweek. While a team holds anyone its squad can't grow: every path that adds a player (bids, auction settlement, trades counted by squad places, loans, recalls, academy/IR promotion) checks `team_is_holding`, and a hold withdraws the team's live bids. If still held at the next gameweek's first kickoff (`held_lineup_lock_at`), the lineup locks and carry-forward repairs the last saved lineup rather than regenerating it. A new roster-status filter must decide what `held` means for it.
- **Academy, IR and loan-out caps are per team, not per league**: a club can buy extra slots (Club Facilities, migration 166), stored as nullable `teams.academy_slots` / `ir_slots` / `loan_out_slots` over the league default. Read them through `effectiveSlots()` in TypeScript or `team_academy_slots()` / `team_ir_slots()` / `team_loan_out_slots()` in SQL, never `leagues.taxi_size` / `ir_size` / `max_loan_outs` directly, or a purchased slot gets refused on one path.
- **IR gates auction bidding independently of its own slot cap**: IR is capped at the team's effective IR slots (league default `leagues.ir_size`, 2, migration 127, enforced in `src/app/api/teams/[teamId]/ir/route.ts`), and separately a team can't place any bid while a healthy player sits on IR — check `roster_status` before allowing a bid, don't just check roster count. Neither rule substitutes for the other.
